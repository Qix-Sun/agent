...
def generate_findus_report(image_paths: str) -> str:
    ...
    response = client.chat.completions.create(
        model="./output/llavaexport",  # 与部署名称一致
        messages=messages,
        max_tokens=20480,  # 增加token限制以适应多图分析
        temperature=0.7
    )
...
fundus_report_tool = Tool(
    name="fundus Report Generator (vLLM)",
    func=generate_findus_report,
    description=(
        "使用本地图像（只能眼科眼底图图像！）调用通过vLLM部署的LLAVA多模态模型生成医学报告。"
        "只能传入单张图像的路径"
        "通过OpenAI兼容接口传递图像Base64格式。"
    )
)
if __name__ == '__main__':
    UWF_path = "D:\\UWF\\白内障\\13695558\\M1\\20230319114517_31000901_64175.jpg"
    generate_findus_report(UWF_path)